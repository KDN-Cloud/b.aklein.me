...
{% include note.html content="<b>Update (July 2026):</b> The upstream <code>crowdsecurity/sshd-logs</code> parser (v3.1) now handles <code>sshd-session</code> natively alongside <code>sshd</code> in its filter. If you are running a current CrowdSec install with an up-to-date collection, you likely do not need the shim described here. That said, not everyone upgrades on a schedule, and plenty of homelab installs are running versions that predate the fix. If you hit this problem and your collection is behind, the approach below still works. The broader technique of a targeted <code>s00-raw</code> rewrite to fix a parser compatibility issue without forking the upstream collection is worth understanding regardless of version." %}
